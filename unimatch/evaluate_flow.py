...
def inference_flow(model,
                   ...
                   output_path='output', # output/gmflow-scale2-regrefine6-davis
                   padding_factor=8, # 32
                   inference_size=None, 
                   save_flo_flow=False,  # save raw flow prediction as .flo
                   attn_type='swin', 
                   attn_splits_list=None, # 2 8
                   corr_radius_list=None, # -1 4
                   prop_radius_list=None, # -1 1
                   num_reg_refine=1, # 6
    ...
    else: # data loading for inference
    ...
    for test_id in range(0, len(filenames) - 1):
        ...
        flow_pr = results_dict['flow_preds'][-1]  # [B, 2, H, W] # 최종 예측된 optical flow map

        # resize back
        if inference_size[0] != ori_size[0] or inference_size[1] != ori_size[1]:
            flow_pr = F.interpolate(flow_pr, size=ori_size, mode='bilinear',
                                    align_corners=True) # padding된 이미지를 원래 크기로 복원, 필요한 경우 upsampling을 통해 원래 해상도로 돌려놓음
            flow_pr[:, 0] = flow_pr[:, 0] * ori_size[-1] / inference_size[-1]
            flow_pr[:, 1] = flow_pr[:, 1] * ori_size[-2] / inference_size[-2]
        ...
        flow = flow_pr[0].permute(1, 2, 0).cpu().numpy()  # [H, W, 2] # 최종적으로 optical flow값을 [H,W,2] 형식으로 변환하여 얻음
        ...
        if pred_bidir_flow:
            ...
            flow_bwd = flow_pr[1].permute(1, 2, 0).cpu().numpy()  # [H, W, 2] # 각 pixel에 대해 [x_offset: x축 이동량, y_offset: y축 이동량]
